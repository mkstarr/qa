...
public class CkEditor extends AbstractTestModule {
    /** @param ide */
    public CkEditor(com.codenvy.ide.IDE ide) {
        super(ide);
    }
    ...
    private interface Locators {
        ...
        String IS_SWITCH_ON_CKEEDITOR =
                "//div[@view-id ='editor-%s']//div[@component='Border']/div/div/div/div[@style='position: absolute; "
                +
                "overflow: hidden; z-index: 0; height: 100%; left: 0px; top: 0px; right: 0px; bottom: 0px; "
                +
        ...
        String CK_EDITOR_CREATED_TABLE =
                "//body[@class='cke_show_borders']//table[@cellspacing='1' and @cellpadding='1' and " +
                "@style]/tbody/tr[%s]/td[%s]";

        String CONTEXT_MAIN_MENU_IFRAME = "//div[contains(@class, 'cke_panel')]/iframe";
        ...
        new WebDriverWait(driver(), 30).until(new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver driver) {
                try {
                    WebElement bar =
                            driver().findElement(
                                    By.cssSelector(String.format(Locators.CK_EDITOR_TOOLS_BAR,
                                                                 getActiveCkEditorTabIndex())));
        ...
        new WebDriverWait(driver(), 30).until(new ExpectedCondition<Boolean>() {
            ...
            public Boolean apply(WebDriver driver) {
                try {
                    ...
                    return getTextFromCKEditor().contains(text);
                } catch (Exception e) {
                    return false;
                }
            }
    ...
    /** wait Ck Editor is empty */
    public void waitIsCkEditorClear() {
        new WebDriverWait(driver(), 30).until(new ExpectedCondition<Boolean>() {

            @Override
            public Boolean apply(WebDriver driver) {
                try {
                    return getTextFromCKEditor().isEmpty();
                } catch (Exception e) {
                    return false;
                }
            }
    ...
    public void typeTextIntoCkEditor(String text) throws Exception {
        WebElement ckiframe =
                driver().findElement(
                        By.cssSelector(String.format(Locators.CK_EDITOR_IFRAME, getActiveCkEditorTabIndex())));
        driver().switchTo().frame(ckiframe);
    ...
    public void selectIFrameWithEditor() throws Exception {
        WebElement ckiframe = driver().findElement(By.cssSelector(String.format(Locators.CK_EDITOR_IFRAME, getActiveCkEditorTabIndex())));
        driver().switchTo().frame(ckiframe);
    }
    ...
    public void waitTablePresent(int row, int cell) {
        new WebDriverWait(driver(), 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(
                Locators.CK_EDITOR_CREATED_TABLE,
                row, cell))));
    }
     ...
     *
     */
    public void switchToContextMenuIframe() throws Exception {
        new WebDriverWait(driver(), 10).until(ExpectedConditions.visibilityOf(contextMainMenuFrame));
    ...
    public void switchToContextSubMenuIframe() {
        new WebDriverWait(driver(), 10).until(ExpectedConditions.visibilityOf(contextSubMenuFrame));
    ...
    public void waitContextSubMenu(final String name) {
        new WebDriverWait(driver(), 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(
                Locators.CONTEXT_SUB_MENU_ELEMENT,
                name))));
    }
    ...
    public void waitContextMenu(final String name) {
        new WebDriverWait(driver(), 30).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(String.format(
                Locators.CONTEXT_MAIN_MENU_ELEMENT,
                name))));
    }
